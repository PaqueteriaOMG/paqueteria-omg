...
export interface ShipmentAttributes {
  ...
  real_delivery_date?: Date | null;
  is_active: number;
  created_at?: Date;
  updated_at?: Date;
}

export type ShipmentCreationAttributes = Optional<ShipmentAttributes, 'shipment_id' | 'package_id' | 'origin_address' | 'destination_address' | 'estimated_delivery_date' | 'real_delivery_date' | 'created_at' | 'updated_at'>;
...
export class Shipment extends Model<ShipmentAttributes, ShipmentCreationAttributes> implements ShipmentAttributes {
  ...
  public real_delivery_date?: Date | null;
...
export const initShipmentModel = (sequelize: Sequelize) => {
  Shipment.init({
    ...
    real_delivery_date: { type: DataTypes.DATE, field: 'ship_real_delivery_date' },
